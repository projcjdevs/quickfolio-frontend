...
import { convertBackendToFrontend } from '@/utils/portfolioConverter';
  ...
  useEffect(() => {
    ...
    const loadPreviewData = async () => {
      try {
        ...
        if (portfolioId) {
          // Load from backend using URL parameter
          try {
            const portfolioDetails = await portfolioApi.getPortfolioDetails(parseInt(portfolioId));
            const { data, templateId: template } = convertBackendToFrontend(portfolioDetails);
            portfolioData = data;
            templateId = template;
          } catch (backendErr) {
            console.error('Failed to load from backend:', backendErr);
            throw new Error('Failed to load portfolio from server');
          }
        } else {
          // Check for portfolio ID in localStorage (from preview button)
          const localPortfolioId = localStorage.getItem('previewPortfolioId');
          if (localPortfolioId) {
            try {
              const portfolioDetails = await portfolioApi.getPortfolioDetails(parseInt(localPortfolioId));
              const { data, templateId: template } = convertBackendToFrontend(portfolioDetails);
              portfolioData = data;
              templateId = template;
            } catch (backendErr) {
              // Fall back to localStorage preview data if backend fails
            ...
            // Fall back to localStorage preview data only
            const saved = localStorage.getItem('preview-data');
            if (saved) {
              const parsed = JSON.parse(saved);
              portfolioData = parsed.data;
        ...
        setError(e instanceof Error ? e.message : 'Failed to load portfolio preview');
      } finally {
        setIsLoading(false);
      }
    };
