...
import { convertBackendToFrontend, createPortfolioContentRequest } from "@/utils/portfolioConverter";
...
export default function PortfolioEditor({ initialTemplate = 'cosmic', portfolioId }: PortfolioEditorProps) {
  ...
  const [portfolioData, setPortfolioData] = useState<PortfolioData>(() => {
    // Only load from localStorage if we're not loading an existing portfolio
  ...
  // Load existing portfolio data if portfolioId is provided
  useEffect(() => {
    const loadPortfolio = async () => {
      ...
      try {
        ...
        const portfolioDetails = await portfolioApi.getPortfolioDetails(portfolioId);
        setPortfolioName(portfolioDetails.name);
        
        // Convert backend data to frontend format
        const { data, templateId } = convertBackendToFrontend(portfolioDetails);
  ...
  const handleSave = async () => {
    try {
      if (portfolioId) {
        ...
        const contentRequest = createPortfolioContentRequest(portfolioData, selectedTemplate);
        await portfolioApi.setPortfolioContent(portfolioId, contentRequest);
      } else {
        // Create new portfolio (this flow might not be used since we use PortfolioGrid for creation)
        ...
        // Set the content
        const contentRequest = createPortfolioContentRequest(portfolioData, selectedTemplate);
        await portfolioApi.setPortfolioContent(newPortfolio.id, contentRequest);
        
        // Update URL to include the new portfolio ID
        window.history.replaceState(null, '', `/edit?portfolioId=${newPortfolio.id}`);
      }
      ...
      alert("Failed to save portfolio. Changes saved locally instead.");
      localStorage.setItem("portfolio-data", JSON.stringify(portfolioData));
    }
  };
